...
const initiateInventory = (maxSize = 3) => {
  WA.player.state.inventoryMaxSize = maxSize
...
const addToInventory = (item: InventoryItem) => {
  const maxSize: number = WA.player.state.inventoryMaxSize as unknown as number
  ...
  if (!hasItem(item.id) && currentInventory.length < maxSize) {
...
const getMaxSize = () => {
  return WA.player.state.inventoryMaxSize
}
